...
const T = {
  ...
  call: ["Ll\xE1manos \xB7 (808) 492-8294", "Call us \xB7 (808) 492-8294"],
...
function UploadModal({ onClose, onSuccess, lang }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => {
        onSuccess && onSuccess({ pet_name: petName, story, submitter_name: anon ? null : name, is_anonymous: anon, photo_url: preview, created_at: (/* @__PURE__ */ new Date()).toISOString() });
        onClose();
      }, 2200);
    } catch (e) {
...
function CommunityStories({ lang }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSuccess(true);
      addStory({ pet_name: petName, story, submitter_name: anon ? null : name, is_anonymous: anon, photo_url: preview, created_at: (/* @__PURE__ */ new Date()).toISOString() });
    } catch (e) {
      setError(tl2("Error al enviar. Intenta de nuevo.", "Error submitting. Please try again."));
    } finally {
      setLoading(false);
