...
function BookingCalendar() {
  ...
  const [useFreeBath, setUseFreeBath] = useState(false);
  ...
  // Identifica el "bano completo" como el servicio a redimir
  const freeBathService = useMemo(() => {
    return [...selectedServices].find((n) => /ba.?o/i.test(n));
  }, [selectedServices]);
  const canRedeemBath = isMember && !!freeBathService;
  const total = useMemo(() => {
    ...
    let credit = 0;
    if (canRedeemBath && useFreeBath) {
      const sv = SERVICES.find((s) => s.name === freeBathService);
      credit = sv ? sv.prices[sizeKey] : 0;
    }
    const pickupCost = pickup && !isMember ? 20 : 0;
    return Math.max(0, base - credit + pickupCost);
  }, [selectedServices, sizeKey, pickup, isMember, useFreeBath, canRedeemBath, freeBathService]);
  ...
  const canConfirm = day && time && petName && ownerName && phone && /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email);
  const canSubmit = canConfirm && accepted;
  // Devuelve si un horario ya pasó o queda con menos de 6h de antelación (para el dia seleccionado)
  const isTimePast = (timeStr) => {
    if (!day) return false;
    const m = String(timeStr || "").match(/(\d+):(\d+)\s*(AM|PM)/i);
    if (!m) return false;
    let h = parseInt(m[1]) % 12; if (/pm/i.test(m[3])) h += 12;
    const slotDate = new Date(year, month, day, h, parseInt(m[2]), 0);
    return (slotDate.getTime() - Date.now()) / 3600000 < 6;
  };
  const buildWhatsApp = () => {
...
\u{1F4DE} Tel\xE9fono: ${phone}${promoCode ? "\n\u{1F3AB} C\xF3digo promo: " + promoCode : ""}${notes ? "\n\u{1F4DD} Notas: " + notes : ""}`);
  ...
  const submitBooking = async () => {
    ...
    try {
      ...
      const notesFull = notes + (zip ? " | ZIP: " + zip + (distInfo ? " (~" + distInfo.miles + "mi)" : "") : "") + (canRedeemBath && useFreeBath ? " | REDIMIR_BANO_MEMBRESIA: " + freeBathService + " (verificar credito en CRM)" : "");
  ...
  })), day && /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: "var(--ink)", marginBottom: 8 } }, "Hora disponible"), /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 6, marginBottom: 16 } }, TIMES.map((t) => { const full = takenSlots.includes(t); const past = isTimePast(t); const blocked = full || past; return /* @__PURE__ */ React.createElement("button", { key: t, disabled: blocked, onClick: () => !blocked && setTime(t), title: full ? "Horario lleno" : past ? "Necesita 6h de antelacion" : "", style: { padding: "8px 4px", borderRadius: 10, border: `2px solid ${time === t ? "var(--orange)" : "var(--line)"}`, background: blocked ? "var(--paper)" : time === t ? "rgba(245,130,32,0.07)" : "var(--bg)", cursor: blocked ? "not-allowed" : "pointer", fontFamily: "inherit", fontSize: 12, fontWeight: time === t ? 700 : 500, color: blocked ? "var(--ink-soft)" : "var(--ink)", textDecoration: blocked ? "line-through" : "none", opacity: blocked ? 0.45 : 1, transition: "all .15s" } }, t); }))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setStep(1), style: { flex: 1, padding: "13px", borderRadius: 14, border: "1.5px solid var(--line)", background: "none", fontFamily: "inherit", fontSize: 14, fontWeight: 600, color: "var(--ink-2)", cursor: "pointer" } }, "\u2190 Atr\xE1s"), /* @__PURE__ */ React.createElement("button", { onClick: () => day && time && setStep(3), disabled: !day || !time, style: { flex: 2, padding: "13px", borderRadius: 14, border: "none", background: day && time ? "var(--orange)" : "var(--line)", color: day && time ? "#fff" : "var(--ink-2)", fontFamily: "inherit", fontSize: 14, fontWeight: 700, cursor: day && time ? "pointer" : "default", transition: "all .2s" } }, "Continuar \u2192"))), step === 3 && /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h3", { style: { fontFamily: "Bricolage Grotesque,sans-serif", fontSize: 18, fontWeight: 700, color: "var(--ink)", margin: "0 0 4px" } }, "Confirmar cita"), /* @__PURE__ */ React.createElement("div", { style: { background: "var(--bg)", borderRadius: 12, padding: "12px 16px", marginBottom: 18, fontSize: 13, color: "var(--ink-2)", lineHeight: 1.7 } }, /* @__PURE__ */ React.createElement("strong", { style: { color: "var(--ink)" } }, [...selectedServices].join(" + ")), " \xB7 ", size, /* @__PURE__ */ React.createElement("br", null), "\u{1F4C5} ", day, " de ", ["Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"][month], ", ", year, " a las ", time), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 10, marginBottom: 18 } }, [
