  ...
  return (
    <div className="bs-fade" style={{ background:BS.bg, minHeight:'100%' }}>
      ...
      <div style={{ padding:'12px 16px 0', display:'flex', gap:8, flexWrap:'wrap' }}>
        {canCreate && <button onClick={()=>{ setCreating(c=>!c); setPermPanel(false); setMsg(''); }} className="bs-btn" style={{ padding:'9px 14px', borderRadius:11, border:'none', background:BS.grad, color:'#fff', fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>{creating ? t(['Cancelar','Cancel']) : t(['+ Crear evento','+ Create event'])}</button>}
        {isAdmin && <button onClick={()=>{ setPermPanel(p=>!p); setCreating(false); setMsg(''); }} className="bs-btn" style={{ padding:'9px 14px', borderRadius:11, border:`1.5px solid ${BS.border}`, background:BS.surface2, color:BS.ink, fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>{t(['Permisos','Permissions'])}</button>}
      </div>
      {msg && <div style={{ padding:'8px 16px 0', color:BS.brand, fontSize:12.5, fontWeight:600 }}>{msg}</div>}
      {creating && (
        <div style={{ margin:'12px 16px 0', padding:'14px', background:BS.surface, borderRadius:14, border:`1px solid ${BS.border}` }}>
          <input value={form.title} onChange={e=>setForm({...form,title:e.target.value})} placeholder={t(['Título del evento','Event title'])} style={fld}/>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
            <select value={form.type} onChange={e=>setForm({...form,type:e.target.value})} style={{...fld, marginBottom:8}}>{TYPES.map(x=><option key={x.id} value={x.id}>{x.label}</option>)}</select>
            <input value={form.event_date} onChange={e=>setForm({...form,event_date:e.target.value})} placeholder={t(['Fecha (ej. 15 Mar, 3pm)','Date (e.g. Mar 15, 3pm)'])} style={fld}/>
          </div>
          <input value={form.location} onChange={e=>setForm({...form,location:e.target.value})} placeholder={t(['Lugar / ciudad','Place / city'])} style={fld}/>
          <input value={form.address} onChange={e=>setForm({...form,address:e.target.value})} placeholder={t(['Dirección (opcional)','Address (optional)'])} style={fld}/>
          <textarea value={form.description} onChange={e=>setForm({...form,description:e.target.value})} placeholder={t(['Descripción','Description'])} rows={3} style={{...fld, resize:'vertical'}}/>
          <input value={form.link} onChange={e=>setForm({...form,link:e.target.value})} placeholder={t(['Link (registro/info, opcional)','Link (signup/info, optional)'])} style={fld}/>
          <input value={form.image_url} onChange={e=>setForm({...form,image_url:e.target.value})} placeholder={t(['URL de imagen (opcional)','Image URL (optional)'])} style={fld}/>
          <button onClick={saveEvent} className="bs-btn" style={{ width:'100%', padding:'11px', borderRadius:11, border:'none', background:BS.grad, color:'#fff', fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>{t(['Publicar evento','Publish event'])}</button>
        </div>
      )}
      {permPanel && isAdmin && (
        <div style={{ margin:'12px 16px 0', padding:'14px', background:BS.surface, borderRadius:14, border:`1px solid ${BS.border}` }}>
          <div style={{ fontSize:13, fontWeight:700, color:BS.ink, marginBottom:8 }}>{t(['Dar permiso para crear','Grant creation permission'])}</div>
          <input value={pf.username} onChange={e=>setPf({...pf,username:e.target.value})} placeholder={t(['Usuario (@username)','Username (@username)'])} style={fld}/>
          <label style={{ display:'flex', alignItems:'center', gap:8, fontSize:13, color:BS.ink2, marginBottom:6 }}><input type="checkbox" checked={pf.can_events} onChange={e=>setPf({...pf,can_events:e.target.checked})}/> {t(['Puede crear eventos','Can create events'])}</label>
          <label style={{ display:'flex', alignItems:'center', gap:8, fontSize:13, color:BS.ink2, marginBottom:10 }}><input type="checkbox" checked={pf.can_news} onChange={e=>setPf({...pf,can_news:e.target.checked})}/> {t(['Puede crear noticias','Can create news'])}</label>
          <button onClick={grant} className="bs-btn" style={{ width:'100%', padding:'10px', borderRadius:11, border:'none', background:BS.grad, color:'#fff', fontSize:13.5, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>{t(['Guardar permiso','Save permission'])}</button>
        </div>
      )}
      <div style={{ padding:'14px 16px 22px', display:'flex', flexDirection:'column', gap:14 }}>
        {!events.length && <div style={{ textAlign:'center', color:BS.soft, padding:'40px 20px', fontSize:13.5, lineHeight:1.6 }}>{t(['Aún no hay eventos. Vuelve pronto o, si tienes permiso, crea el primero.','No events yet. Check back soon, or create the first one if you have permission.'])}</div>}
        {events.map(ev => {
          ...
          return (
            <div key={ev.id} className="bs-pop" style={{ background:BS.surface, borderRadius:18, overflow:'hidden', border:`1px solid ${BS.border}` }}>
              {ev.image_url ? <img src={ev.image_url} alt="" onClick={()=>setSel(ev)} style={{ width:'100%', height:150, objectFit:'cover', display:'block', cursor:'pointer' }} loading="lazy"/> : <div onClick={()=>setSel(ev)} style={{ height:90, background:BS.grad, cursor:'pointer' }}/>}
              <div style={{ padding:'14px 16px' }}>
                <div style={{ display:'inline-block', fontSize:10.5, fontWeight:700, color:BS.brand, background:'rgba(245,130,32,0.1)', borderRadius:999, padding:'2px 9px', marginBottom:6 }}>{typeLabel(ev.type)}</div>
                <div onClick={()=>setSel(ev)} style={{ fontFamily:'Bricolage Grotesque,sans-serif', fontSize:16.5, fontWeight:800, color:BS.ink, lineHeight:1.25, marginBottom:5, cursor:'pointer' }}>{ev.title}</div>
                {ev.event_date && <div style={{ fontSize:12.5, color:BS.brand, fontWeight:700, marginBottom:2 }}>{ev.event_date}</div>}
                {ev.location && <div style={{ fontSize:12, color:BS.soft, marginBottom:12 }}>{ev.location}</div>}
                <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                  <button onClick={() => toggle(ev.id)} className="bs-btn" style={{ padding:'9px 18px', borderRadius:11, border:'none', background: on?BS.surface2:BS.grad, color: on?BS.ink:'#fff', fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>{on?t(['Asistirás ✓','Going ✓']):t(['Asistir','Attend'])}</button>
                  <button onClick={()=>setSel(ev)} className="bs-btn" style={{ background:'none', border:'none', color:BS.brand, fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>{t(['Ver detalle →','View details →'])}</button>
                </div>
              </div>
            </div>
          );
      ...
      {sel && (
        <div onClick={()=>setSel(null)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', zIndex:60, display:'flex', alignItems:'flex-end', justifyContent:'center' }}>
          <div onClick={e=>e.stopPropagation()} style={{ background:BS.surface, borderRadius:'18px 18px 0 0', width:'100%', maxWidth:480, maxHeight:'85%', overflow:'auto' }}>
            {sel.image_url && <img src={sel.image_url} alt="" style={{ width:'100%', height:180, objectFit:'cover', display:'block' }}/>}
            <div style={{ padding:'16px 16px 24px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:10 }}>
                <div style={{ fontFamily:'Bricolage Grotesque,sans-serif', fontSize:20, fontWeight:800, color:BS.ink }}>{sel.title}</div>
              ...
              <div style={{ display:'inline-block', fontSize:10.5, fontWeight:700, color:BS.brand, background:'rgba(245,130,32,0.1)', borderRadius:999, padding:'2px 9px', margin:'6px 0' }}>{typeLabel(sel.type)}</div>
              {sel.event_date && <div style={{ fontSize:13, color:BS.brand, fontWeight:700 }}>{sel.event_date}</div>}
              {sel.location && <div style={{ fontSize:12.5, color:BS.soft, marginTop:2 }}>{sel.location}{sel.address?(' · '+sel.address):''}</div>}
              {sel.description && <div style={{ fontSize:14, color:BS.ink, marginTop:10, lineHeight:1.6, whiteSpace:'pre-wrap' }}>{sel.description}</div>}
              <div style={{ display:'flex', gap:10, marginTop:16, flexWrap:'wrap' }}>
                <button onClick={()=>toggle(sel.id)} className="bs-btn" style={{ flex:1, padding:'11px', borderRadius:11, border:'none', background: going.has(sel.id)?BS.surface2:BS.grad, color: going.has(sel.id)?BS.ink:'#fff', fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>{going.has(sel.id)?t(['Asistirás ✓','Going ✓']):t(['Asistir','Attend'])}</button>
                {sel.link && <a href={sel.link} target="_blank" rel="noopener noreferrer" className="bs-btn" style={{ flex:1, textAlign:'center', padding:'11px', borderRadius:11, border:`1.5px solid ${BS.border}`, background:BS.surface2, color:BS.ink, fontSize:14, fontWeight:700, textDecoration:'none', fontFamily:'inherit' }}>{t(['Más info →','More info →'])}</a>}
              </div>
              {(sel.lat!=null && sel.lng!=null) && <a href={'https://www.google.com/maps/search/?api=1&query='+sel.lat+','+sel.lng} target="_blank" rel="noopener noreferrer" style={{ display:'block', textAlign:'center', marginTop:10, fontSize:12.5, fontWeight:700, color:BS.brand }}>{t(['Cómo llegar','Directions'])} {'→'}</a>}
            </div>
          </div>
        </div>
      )}
